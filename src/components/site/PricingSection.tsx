        ...
        {allInclusive.length > 0 && (
          <>
            <Reveal className="mx-auto mt-20 max-w-xl text-center">
              <p className="mb-3 text-xs font-semibold uppercase tracking-[0.25em] text-[#C6A15B]">All-Inclusive Packages</p>
              <h2 className="font-serif text-3xl font-light leading-[1.1] text-[#26221C] sm:text-4xl">Everything You Need, One Price</h2>
              <p className="mt-4 text-[#26221C]/60">Skip the planning. Accommodation, tours, meals, and transport — bundled.</p>
            </Reveal>
              ...
              {allInclusive.map((pkg, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Pricing Tiers */}
                    <div className={cn("mt-6 space-y-2 rounded-2xl p-4", pkg.featured ? "bg-white/5" : "bg-[#26221C]/5")}>
                      <div className="flex items-baseline justify-between">
                        <span className={cn("text-sm", pkg.featured ? "text-[#F5EFE2]/60" : "text-[#26221C]/50")}>1 Person</span>
                        <div className="flex items-baseline gap-1">
                          <span className="font-serif text-3xl">{formatPrice(pkg.price)}</span>
                          <span className={cn("text-xs", pkg.featured ? "text-[#F5EFE2]/40" : "text-[#26221C]/35")}>/ {pkg.period}</span>
                        </div>
                      </div>
                      {pkg.priceTwo > 0 && (
                        <div className="flex items-baseline justify-between">
                          ...
                          <div className="flex items-baseline gap-1">
                            <span className="font-serif text-3xl">{formatPrice(pkg.priceTwo)}</span>
                            <span className={cn("text-xs", pkg.featured ? "text-[#F5EFE2]/40" : "text-[#26221C]/35")}>/ {pkg.period}</span>
                          </div>
                        </div>
                      )}
