...
export function useSpeechInput(
  ...
  const stop = useCallback(() => {
    ...
    // Do not suppress finalization here. A manual Stop on iOS commonly arrives
    // while the transcript is still interim; onend must submit that text.
    try {
      rec.stop();
    } catch {
  ...
  const start = useCallback(() => {
    ...
    // A stale WebKit recognizer may still exist after a quick close/reopen.
    // Abort it before creating the next session so old onend callbacks cannot
    // submit into the new turn.
    if (recRef.current) {
      try {
    ...
    rec.onresult = (event) => {
      ...
      // Desktop Chrome may wait another ~1s after producing a final result.
      // Stop immediately. Mobile browsers that never mark a result final are
      // handled by the latestTranscriptRef fallback in onend below.
      if (gotFinal) {
        try {
          rec.stop();
        } catch {
    ...
    rec.onerror = (event) => {
      ...
      // Permission/network/abort failures must never accidentally submit a
      // stale interim transcript when WebKit fires onend immediately after.
    ...
    try {
      // Set the synchronous ref before start() so a rapid double tap cannot
      // create two recognizers before React commits listening=true.
