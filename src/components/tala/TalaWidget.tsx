      ...
      {open && (
        ...
        >
          <div className="flex items-center gap-3 px-4 py-3 text-white" style={{ background: `linear-gradient(135deg, ${GREEN} 0%, ${GREEN_DARK} 100%)` }}>
            ...
            <button onClick={() => voice.enabled ? (voice.stop(), voice.setEnabled(false)) : voice.setEnabled(true)} aria-label={voice.enabled ? "Turn voice off" : "Turn voice on"} className="rounded-full p-2 transition-colors hover:bg-white/10">
              {voice.enabled ? <Volume2 className="h-4 w-4" style={{ color: GOLD }} /> : <VolumeX className="h-4 w-4 text-white/60" />}
            </button>
            <button onClick={() => setShowSettings((s) => !s)} aria-label="TALA settings" className="rounded-full p-2 transition-colors hover:bg-white/10"><Settings2 className="h-4 w-4 text-white/80" /></button>
            <button onClick={() => { voice.stop(); speech.abort(); setIntent(null); setOpen(false); }} aria-label="Close TALA" className="rounded-full p-2 transition-colors hover:bg-white/10"><X className="h-4 w-4 text-white/80" /></button>
          </div>
          ...
          {showSettings && (
            <div className="border-b px-4 py-3 text-xs" style={{ borderColor: `${GOLD}33`, color: INK }}>
              ...
              <div className="mb-3 w-full rounded-md border bg-white/60 px-2 py-1.5" style={{ borderColor: `${GOLD}55` }}>
                {TALA_KOKORO_VOICES.find((v) => v.id === voice.voiceId)?.label ?? voice.voiceId}<span className="ml-1 opacity-50">(set in Admin)</span>
              </div>
              ...
              <input type="password" value={devKey} onChange={(e) => { setDevKeyState(e.target.value); setDevApiKey(e.target.value.trim()); }} placeholder="sk-or-…" className="mb-3 w-full rounded-md border bg-white px-2 py-1.5" style={{ borderColor: `${GOLD}55` }} />
              ...
              <button onClick={() => { voice.stop(); chat.reset(); setIntent(null); setShowSettings(false); }} className="flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 transition-colors hover:bg-white" style={{ borderColor: `${GOLD}55` }}><RotateCcw className="h-3 w-3" /> Clear conversation</button>
            </div>
          )}
          ...
          <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
            ...
            <a href={waHref} target="_blank" rel="noreferrer" className="mx-3 mb-2 flex items-center justify-center gap-2 rounded-full border px-3 py-2 text-xs font-medium transition-colors hover:bg-white" style={{ borderColor: "#25D36688", color: "#1F7A3D", backgroundColor: "#25D36614" }}>
              <MessageCircle className="h-3.5 w-3.5" />{chat.error ? "TALA hit a snag — message us on WhatsApp" : "Prefer a human? Message us on WhatsApp"}
            </a>
          </div>
          ...
          {speech.error && !speech.listening && (
            <div className="flex items-start justify-between gap-2 border-t px-4 py-2 text-[11px]" style={{ borderColor: `${GOLD}33`, backgroundColor: "#FBEFEC", color: "#8C3B32" }}>
              <span className="flex-1">{speech.error}</span><button type="button" onClick={() => speech.start()} className="shrink-0 font-semibold underline">Try again</button>
            </div>
          )}

          <form onSubmit={(e) => { e.preventDefault(); void submit(input); }} className="flex items-center gap-2 border-t px-3 py-3" style={{ borderColor: `${GOLD}33`, backgroundColor: "#FFFFFF" }}>
            {speech.supported && (
            ...
            <input ref={inputRef} value={speech.listening ? speech.transcript : input} onChange={(e) => { if (voice.status === "speaking") voice.stop(); setInput(e.target.value); }} readOnly={speech.listening} placeholder={speech.listening ? "Listening…" : "Ask TALA anything…"} className="min-w-0 flex-1 rounded-full border px-4 py-2 text-sm outline-none focus:ring-2" style={{ borderColor: `${GOLD}55`, color: INK }} />
            <button type="submit" disabled={chat.thinking || (!input.trim() && !speech.listening)} aria-label="Send" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-white transition-transform active:scale-95 disabled:opacity-40" style={{ backgroundColor: GREEN }}><Send className="h-4 w-4" /></button>
          </form>
        </div>
      )}
    </>
...
function Bubble({ role, text }: { role: "user" | "assistant"; text: string }) {
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div className="max-w-[85%] whitespace-pre-wrap rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed shadow-sm" style={isUser ? { backgroundColor: GREEN, color: "#FFFFFF", borderBottomRightRadius: 6 } : { backgroundColor: "#FFFFFF", color: INK, border: `1px solid ${GOLD}33`, borderBottomLeftRadius: 6 }}>{text}</div>
    </div>
  );
}
...
function BookingFormCard({ draft, tours, onConfirm, onEdit }: {
  draft: { id: string; reference: string; guestName: string; guestPhone: string; roomType: string; checkIn: string; checkOut: string; guests: number; amount: number; notes: string };
  tours: { name: string }[];
  ...
  const toggleTour = (name: string) => setPicked((p) => p.includes(name) ? p.filter((x) => x !== name) : [...p, name]);
  const nights = Math.max(0, Math.round((new Date(draft.checkOut).getTime() - new Date(draft.checkIn).getTime()) / 86400000)) || 1;

  return (
    <div className="mx-1 rounded-xl border-2 px-3.5 py-3" style={{ borderColor: GOLD, backgroundColor: "#FFFFFF", color: INK }}>
      <p className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide" style={{ color: GOLD }}><Sparkles className="h-3.5 w-3.5" /> Your booking — please confirm</p>
      ...
      <div className="mt-3 space-y-2.5">
        <label className="block text-sm"><span className="opacity-70">Email</span><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 w-full rounded-md border px-2 py-1.5 text-sm" style={{ borderColor: `${GOLD}55` }} /></label>
        <label className="block text-sm"><span className="opacity-70">WhatsApp / Phone</span><input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="mt-1 w-full rounded-md border px-2 py-1.5 text-sm" style={{ borderColor: `${GOLD}55` }} /></label>
        <div className="flex gap-2">
          <button type="button" onClick={() => setNomad((v) => !v)} className="flex-1 rounded-full px-2.5 py-1.5 text-[11px] font-medium" style={nomad ? { backgroundColor: GREEN, color: "#fff" } : { border: `1px solid ${GOLD}55`, color: INK }}>Digital nomad</button>
          <button type="button" onClick={() => setWorking((v) => !v)} className="flex-1 rounded-full px-2.5 py-1.5 text-[11px] font-medium" style={working ? { backgroundColor: GREEN, color: "#fff" } : { border: `1px solid ${GOLD}55`, color: INK }}>Working here</button>
        </div>
        {tours.length > 0 && <div><p className="mb-1 text-[11px] opacity-70">Tours you might like</p><div className="flex flex-wrap gap-1.5">{tours.map((t) => <button key={t.name} type="button" onClick={() => toggleTour(t.name)} className="rounded-full px-2.5 py-1 text-[11px] font-medium" style={picked.includes(t.name) ? { backgroundColor: GOLD, color: "#fff" } : { border: `1px solid ${GOLD}55`, color: INK }}>{t.name}</button>)}</div></div>}
      </div>
      <p className="mt-2 text-[11px] opacity-60">Pending — the team confirms after you submit.</p>
      <div className="mt-3 flex gap-2">
        <button type="button" onClick={() => onConfirm({ email: email.trim() || undefined, phone: phone.trim() || undefined, nomad, working, tours: picked })} className="flex-1 rounded-full py-2 text-xs font-semibold text-white" style={{ backgroundColor: GREEN }}>Confirm booking</button>
        <button type="button" onClick={onEdit} className="rounded-full border px-3 py-2 text-xs font-medium" style={{ borderColor: `${GOLD}55` }}>Edit</button>
      </div>
    </div>
  );
}
